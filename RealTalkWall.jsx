...
export default function RealTalkWall() {
  ...
  const handleAction = (type) => {
    setAction(type);
    alert(`你的話已被「${type}」處理，我們會代你完成後續流程。`);
  };
  ...
  return (
    <div className="max-w-xl mx-auto mt-10 p-4">
      ...
      {!submitted ? (
        ...
        <div className="space-y-4">
          <p className="font-semibold">你說了：</p>
          <p className="bg-gray-100 p-3 rounded-md">{post}</p>
          <p className="text-sm text-gray-500">你希望我們怎麼處理這句話？</p>
          <div className="grid grid-cols-3 gap-2">
            <button onClick={() => handleAction('寄出')} className="border rounded py-1">寄出</button>
            <button onClick={() => handleAction('展示')} className="border rounded py-1">展示</button>
            <button onClick={() => handleAction('銷毀')} className="bg-red-600 text-white rounded py-1">銷毀</button>
          </div>
        </div>
      )}
    </div>
